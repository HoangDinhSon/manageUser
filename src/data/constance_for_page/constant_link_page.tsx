//1. Page Account
const LINK_PAGE_ACCOUNT = '/accounts';
export const PAGE_ACCOUNT = 'accounts';
const LINK_PAGE_PARTNER = '/accounts/partner';
const LINK_PAGE_VINOVA = '/accounts/vinova';
const LINK_PAGE_ACCOUNT_EDIT = '/accounts/edit';
const LINK_PAGE_ACCOUNT_ADD = '/accounts/add';
const LINK_PAGE_ACCOUNT_FILTER = '/accounts/filter';
//2. Page Project 
export const PAGE_DASHBOARD = 'dashboard';
export const PAGE_PROJECT = 'project';
export const PAGE_ROLEMANAGER = 'rolemanager';
export const PAGE_STACKS = 'stacks';
export const PAGE_REPORT = 'report';
const LINK_PAGE_DASHBOARD = '/dashboard';
const LINK_PAGE_PROJECT = '/project';
const LINK_PAGE_ROLEMANAGER = '/rolemanager';
const LINK_PAGE_STACKS = '/stacks';
const LINK_PAGE_REPORT = '/report';
const LINK_REPORT_WATCH = ':id/watch';
const LINK_REPORT_EDIT = ':id/edit';
export {
    LINK_PAGE_ACCOUNT_EDIT,
    LINK_PAGE_ACCOUNT_ADD,
    LINK_PAGE_ACCOUNT_FILTER,
    LINK_PAGE_ACCOUNT,
    LINK_PAGE_DASHBOARD,
    LINK_PAGE_PROJECT,
    LINK_PAGE_STACKS,
    LINK_PAGE_ROLEMANAGER,
    LINK_PAGE_REPORT,
    LINK_REPORT_WATCH,
    LINK_REPORT_EDIT,
    LINK_PAGE_PARTNER,
    LINK_PAGE_VINOVA,
};
